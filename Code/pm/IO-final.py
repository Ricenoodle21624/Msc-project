...
Dir_home = os.getcwd()
Dir_result = Dir_home + "/PM/io-test1/pm-io"
...
def savecsv(filename='', starttime='', endtime=''):
    ...
    query = "q=SELECT \"rkB_per_s\", \"wkB_per_s\", \"tps\" FROM \"disk\" WHERE time >= '"+starttime+"' AND time <= '"+endtime+"' tz('Europe/London')"
    #print query
    with open(filename, "a") as output:
...
def main():
    ...
    # File Test
    for i in range(num):
        result = Dir_home + "/PM/io-test1/pm-io" + str(count)
        if((i%8)==0):
