...
Dir_home = os.getcwd()
Dir_result = Dir_home + "/Docker/cpu-test1/Docker-cpu"
...
def savecsv(filename='', starttime='', endtime=''):
    query = "q=SELECT \"usage_percent\" FROM \"docker_container_cpu\" WHERE \"container_name\"='telegraf' AND time >= '"+starttime+"' AND time <= '"+endtime+"'"
    #print query
    with open(filename, "a") as output:
...
def main():
	...
	for i in range(num):
         ...
         Dir_result = Dir_home + "/Docker/cpu-test1/Docker-cpu" + str(count)
